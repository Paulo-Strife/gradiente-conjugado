...
#define DIAG(A, d, i) ((A)->diag[(size_t)(d) * (A)->n + (i)])

// comparado com o diag da primeira versão do programa, esse agora vai guardar a diagonal em um bloco de memória só.
...
bandaMatriz *cria_matriz(int n, int banda) {
    ...
    int qtd_diag = A->meia_banda + 1;

    A->diag = (double *) calloc((size_t) qtd_diag * n, sizeof(double));
...
void free_matrix(bandaMatriz *A) {
    ...
    // aqui removemos um for que era utilizado anteriormente.
...
void gerar_banda_espalhada_matriz(bandaMatriz *A, unsigned int seed) {
    ...
    for (int i = 0; i < n; i++) {
        DIAG(A, 0, i) = seq_soma[i] + rand_double(1.0, 2.0);
    }
