...
SRC_OPT="conjugated_method_restrict.c"

BIN_BASELINE="baseline"
BIN_OPT="opt_restrict"
...
if [ $? -ne 0 ]; then
    echo "Erro ao compilar opt_restrict."
    exit 1
fi
...
echo "versao,N,banda,rep,tempo_s,memoria_kb,cycles,instructions,cache_references,cache_misses,branches,branch_misses" > resultados/benchmark_restrict.csv
...
for versao in baseline opt_restrict
do
...
echo "Arquivo gerado: resultados/benchmark_restrict.csv"